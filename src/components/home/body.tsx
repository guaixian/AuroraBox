        ...
        {!canPickNode && (
          <div className="px-3 py-4 rounded-xl text-center" style={{ background: "var(--aurorabox-fill)" }}>
            <p className="text-xs text-[var(--aurorabox-label-tertiary)]">
              ...
              {active?.group_type === "chain" && `级联: ${members.map((m: any, i: number) => { const s = allServers.find(x => x.identifier === m.server_identifier); return s ? (i+1)+"."+s.name : ""; }).filter(Boolean).join(" → ")}, 出口IP=最后`}
            </p>
          </div>
        )}
      </section>
